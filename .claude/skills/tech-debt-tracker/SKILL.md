...
description: Scan the codebase for technical debt and generate a prioritized markdown report. Use when auditing code quality, preparing sprint cleanup, tracking TODO/FIXME density, or adding Obsidian vault tasks for debt reduction.
---
...
Scans the Strummy codebase for technical debt indicators and produces a prioritized markdown report grouped by severity. Optionally adds tasks to the Obsidian vault inbox for critical and high-severity items.
...
/tech-debt-tracker --create-tickets   # Also add Obsidian vault tasks for critical/high
...
### Phase 4: Add Obsidian Vault Tasks (optional)

When `--create-tickets` is specified, append tasks to the Obsidian vault inbox for Critical and High items.

Use the Obsidian MCP tool:

```
obsidian_append_content("inbox.md",
  "- [ ] [TECH-DEBT] Fix `: any` type in cron dispatcher — app/api/cron/dispatcher/route.ts:56"
)
```
...
- Critical items first, then High
- One task per item (not grouped)
...
### Full scan with vault task creation

```
/tech-debt-tracker --create-tickets
```

Adds tasks to Obsidian `inbox.md` for all Critical and High items, then prints:

```
Added 5 tasks to Obsidian inbox:
  [TECH-DEBT] Fix `: any` in cron dispatcher (Critical)
  [TECH-DEBT] Split AdminDashboardClient.tsx (312 LOC) (High)
  ...
```
...
| Situation                                  | Action                                                   |
| ------------------------------------------ | -------------------------------------------------------- |
| Obsidian MCP unavailable                   | Skip task creation, print warning, still generate report |
